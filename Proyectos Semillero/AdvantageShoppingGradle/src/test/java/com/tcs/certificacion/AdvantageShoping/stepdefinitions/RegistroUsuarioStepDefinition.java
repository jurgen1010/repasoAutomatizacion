...
import com.tcs.certificacion.AdvantageShopping.models.NombreCampos2;
...
import com.tcs.certificacion.AdvantageShopping.util.drivers.ExceptionElementoNoEncontrado;
...
public class RegistroUsuarioStepDefinition {
	...
	public void elDiligenciaElCampoUsername(List<List<String>>datos) throws InterruptedException  {
	   Thread.sleep(3000);
	...
	public void elDiligenciaElCampoFisrtname(List<List<String>> datos)  {
		//actor.attemptsTo(DiligenciarCampo.conNombre(, valor));
		actor.attemptsTo(DiligenciarCampo.conNombre(NombreCampos.nombreCampo(datos.get(1).get(0)), datos.get(1).get(1)));
	}
	...
	public void elPodraVerElMensajeDeAdvertencia(List<List<String>>datos)  {
		 actor.should(seeThat(VerAdvertenciaLongitudCampos.permitida(), is(datos.get(1).get(0)))
				 .orComplainWith(ExceptionElementoNoEncontrado.class, "Elemento no encontrado"));
